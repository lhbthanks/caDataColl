...
function search(){
	...
	var url = "<c:url value="/suzuki/brakeFluidDurr.do?m=list"/>";
	if(vin_code.value!=""){
...
function exportExcel(){
	...
	var mobile_unit = document.getElementById("mobile_unit");
	var filling_result = document.getElementById("filling_result");
	var url = "<c:url value="/suzuki/brakeFluidDurr.do?m=listExcel"/>";
	if(vin_code.value!=""){
		url+="&vin_code="+vin_code.value;
	}
	if(car_type.value!=""){
	...
	<div class="wrap">
		...
		<div class="main">
			...
			<div id="instructionalResearch" class="mediumTurquoise"
				...
				<dl>
					<dt>
						<em><a href="<c:url value="#"/>">杜尔刹车油查询</a></em>
					</dt>
				...
				<td valign="top">
					<div id="c01">
						<dl>
							...
							<dt style="padding-right:5px;">
								加注结果：<select id="filling_result" name="filling_result" style="width:100px;" >
								<option value="">全部</option>
						...
						<table width="99%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr>
								...
								<td  height="30" bgcolor="#e5edfa" align="center">时间</td>
								<td  height="30" bgcolor="#e5edfa" align="center">泄露测试</td>
								<td  height="30" bgcolor="#e5edfa" align="center">时间</td>
								<td  height="30" bgcolor="#e5edfa" align="center">循环时间</td>
								<td  height="30" bgcolor="#e5edfa" align="center">ABS_State</td>
							</tr>
							<c:choose>
								<c:when test="${not empty brakeFluidDurrList}">
									<c:forEach items="${brakeFluidDurrList}" var="antifreeze" varStatus="vs">
										<tr>
											...
											<td height="30" bgcolor="#FFFFFF" align="center"> <c:out value="${antifreeze.abs_state}" /></td> 
										</tr>
									</c:forEach>
								</c:when>
								<c:otherwise>
									<tr>
										<td colspan="19" height="30" bgcolor="#ffffff" align="center">暂无数据</td>
									</tr>
								</c:otherwise>
							</c:choose>
						</table>
