...
<div style="float:right;padding-right:20px;">
		<span class="txt"> 
				...
				<c:otherwise>
					<c:choose>
						...
						<c:when test="${(PageInfo.total_page-PageInfo.page)>=0 && (PageInfo.total_page-PageInfo.page) <=5}"> 
							<c:set var="firstPage" value="${PageInfo.total_page-9}" scope="page" />
							<c:set var="endPage" value="${PageInfo.total_page}" scope="page" />
						</c:when>
						
						
						<c:when test="${(PageInfo.total_page-PageInfo.page) >5&&PageInfo.page>5}"> 
							<c:set var="firstPage" value="${PageInfo.page-4}" scope="page" />
							<c:set var="endPage" value="${PageInfo.page+5}" scope="page" />
						</c:when>
						
						
						<c:otherwise>
							<c:set var="firstPage" value="${PageInfo.page+0}" scope="page" />
							<c:set var="endPage" value="${PageInfo.page+9}" scope="page" />
						</c:otherwise>
					</c:choose>
				</c:otherwise> 
			</c:choose> 
			...
			&nbsp;总页数：<c:out value="${PageInfo.total_page}" />
			<br />
			</span>
