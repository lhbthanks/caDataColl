...
<td valign="top">
	<div class="middle">
		<form action="<c:url value="/admin/clazz/clazzMenu.do?m=add"/>" method="post" style="margin:0px;" onsubmit="return check(this)">
		<table width="99%" border="0" cellpadding="0" cellspacing="1" bgcolor="#dae2e5">
			<tr>
				<td width="30%" height="30" bgcolor="#ffffff" align="center">
					父目录: <select name="pid" id="pid" style="width:150px;" onchange="changeclazzmenu(this.value)">
							<option value="0">无</option>
							<c:forEach items="${topMenuList}" var="topMenu">
								<option value="${topMenu.id}" ${param.pid==topMenu.id?'selected=selected':''}><c:out value="${topMenu.name}"/></option>
							</c:forEach>
						   </select>
				</td>
	    	...
	    	<c:when test="${not empty menuList}">
	    		...
	    		<tr>
			      <td width="7%" height="30" bgcolor="#FFFFFF" align="center">
			      	<input type="checkbox" name="menuId" value="${menu.id}"/>&nbsp;<c:out value="${vs.index+1}" />
			       </td>
